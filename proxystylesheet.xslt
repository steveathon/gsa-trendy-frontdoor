...
<xsl:template name="search_results_body">
  ...
  <!-- *** Top separation bar *** -->
	...
	<script language='javascript' src='clicklog.js'></script>
    </xsl:if>
...
<!-- **********************************************************************
	Show the trendy frontdoor from twitter using YQL.
	Written by Steven King <steve@stevenking.com.au>
	*********************************************************************** -->
<xsl:template name="trendy_frontdoor">
	<p>Showing the latest results in our collection matching the global trending topics on <a href='http://twitter.com/' target='_blank'>twitter.com</a></p>
	<div id='emptyFrontDoor'></div>
	<script type='text/javascript'>
	<xsl:comment>
	function emptyLoader(a) {
	{try{if ( a.query.results.R.length &gt; 0 ) {
			var b = document.getElementById('emptyFrontDoor');
			for ( var i = 0; i &lt; a.query.results.R.length; i++ ) {
				b.innerHTML = b.innerHTML + '&lt;p class="g"&gt;&lt;font size="-2"&gt;&lt;b&gt;&lt;/b&gt;&lt;/font&gt; &lt;a ctype="c" rank="1" href="' + a.query.results.R[i].UE + '"&gt;&lt;span class="l"&gt;' + a.query.results.R[i].T + '&lt;/span&gt;' +
				'&lt;/a&gt;&lt;table cellpadding="0" cellspacing="0" border="0"&gt;&lt;tr&gt;&lt;td class="s"&gt;' + a.query.results.R[i].S + '&lt;br /&gt;&lt;font color="#008000" size="-1"&gt;' +
				a.query.results.R[i].U + '&lt;/font&gt;&lt;font color="#008000" size="-1"&gt;' + a.query.results.R[i].CRAWLDATE + '&lt;/font&gt;&lt;/td&gt;&lt;/tr&gt;&lt;/table&gt;&lt;/p&gt;';
			}
		}}catch(e){}}
	}
	</xsl:comment>
	...
	<xsl:text disable-output-escaping="yes">
	&lt;script type='text/javascript' src='</xsl:text><xsl:call-template name="trendy_frontdoor_yql" /><xsl:text disable-output-escaping="yes">'&gt;&lt;/script&gt;</xsl:text>
</xsl:template>

<!-- **********************************************************************
  Trendy JS will load based on the current Collection and Site. Handy, Right?
  Written by Steven King <steve@stevenking.com.au>
  Uses the latest YQL table orignally from: https://github.com/steveathon/yql-tables/blob/master/google/google.searchappliance.search.xml
  Will use the latest production table on https://github.com/yql/yql-tables/
     ********************************************************************** -->
     
<xsl:template name="trendy_frontdoor_yql">
<xsl:text>http://query.yahooapis.com/v1/public/yql?q=</xsl:text><xsl:if test='$trendy_use_testing != 0'><xsl:value-of select="$trendy_use_testing" /></xsl:if>select%20*%20from%20google.searchappliance.search%20where%20appliance%3D%22<xsl:value-of select="$gsa_publicurl" />%22%20and%20q%20in%20(select%20name%20from%20twitter.trends)%20and%20site%3D%22<xsl:value-of select="/GSP/PARAM[@name='site']/@value" />%22%20and%20client%3D%22<xsl:value-of select="/GSP/PARAM[@name='client']/@value" />%22&amp;format=json&amp;diagnostics=false&amp;env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&amp;callback=emptyLoader</xsl:template>
...
<select name="site">
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='cafebones')]">
   <option value="cafebones" selected="selected">cafebones</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="cafebones">cafebones</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
